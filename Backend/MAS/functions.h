...
int batch_overlap_length = 0;
int batch_damaged_lenght = 0;
...
void total_damaged_lenght(float value) {
  batch_damaged_lenght += value;
  Firebase.setDouble(firebaseData, data_base + "/" + String(batch_number) + "/batch_damaged_lenght", batch_damaged_lenght);
}
